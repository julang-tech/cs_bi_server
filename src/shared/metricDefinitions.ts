        ...
        items: [
          {
            ...
            name: '财务三层模型',
            short: 'GMV → 净实付金额 → 净 GMV，从"商品定价"到"实际进账"再到"扣完退款"。',
            detail:
              '看板的 4 个金额指标对应公司财务的三层口径：①GMV（商品定价 + 运费，含所有折扣码 / CS 代金券）→ ②净实付金额（GMV 剔除 CS 代金券，对应客户实际进账）→ ③净 GMV（净实付再剔退款 + 运费按净值）。所有金额都含运费、不含税。',
          },
          {
            id: 'p2.concept_cs_voucher',
            name: 'CS 代金券（客诉代金券）',
            short: '客服因客诉给客户发的补偿券，统计上视为"公司自掏腰包"，不计入收入。',
            detail:
              '正常促销折扣（DiscountCode）算 GMV 的一部分，但 CS 客诉补偿券（Manual Discount）不算公司收入——所以从 GMV 到"净实付金额"会减去 CS 代金券。这样净实付反映的是客户真正掏出来的钱，便于评估退款压力。',
          },
          {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            id: 'p2.gmv',
            name: 'GMV',
            short: '商品定价 + 运费的合计，按美元合计。',
            formula: 'GMV = 商品定价合计（不含税、含 CS 代金券抵扣前金额）+ 运费',
            detail:
              '①层口径：客户下单时商品的"定价"金额合计 + 运费。这里的"定价"指折扣前金额，包含所有折扣（普通折扣码 + CS 代金券）+ 实付的总和——可以理解成"如果没任何折扣，客户该付多少"。不扣退款。',
          },
          {
            id: 'p2.net_received_amount',
            name: '净实付金额',
            short: '剔除 CS 客诉代金券后的实收，含运费，按美元合计。',
            formula: '净实付金额 = 商品实付（含普通折扣后的金额）+ CS 代金券 + 运费',
            detail:
              '②层口径：客户实际刷卡 / 转账掏出来的钱（普通折扣码已抵扣）再加回 CS 客诉代金券（因为代金券是客户没真正花的钱）。是"退款金额占比"的分母——衡量退款相对真实进账的压力。',
          },
          {
            id: 'p2.net_revenue_amount',
            name: '净 GMV',
            short: '净实付再扣退款后的最终收入，按美元合计。',
            formula: '净 GMV = 商品实付 + CS 代金券 + 运费净值 − 退款（仅商品部分）',
            detail:
              '③层口径：在"净实付金额"基础上扣掉退款（仅商品部分），并把运费换成"净运费"（已经扣掉退款单里退的运费）。反映退款后真正留在公司账上的金额。',
          },
          {
